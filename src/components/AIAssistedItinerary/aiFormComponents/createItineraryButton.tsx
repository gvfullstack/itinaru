...
const CreateItineraryButton: React.FC<DefinedProps> = (props) => {
    ...
    const prompt = `For a tourist visiting ${neighborhoodsToExplore.length >0 ? neighborhoods: ""} in ${destination}, we want to generate a travel itinerary. Please provide each of the following as separate JSON objects: please ${sitePrompts} Each suggestion should begin with "{" and end with "}", and follow this structure exactly: "activityType": "<Coffee Shop>", "itemTitle": "<name>", "description": "<description>", "locationAddress": "<address>" This format is like a JSON object. Please, do not add any extra information or labels, and do not include these suggestions in a list or a larger JSON object. The personal preferences of the traveler are as follows: ${itinPreferences}` 
    ...
    return (
        <div className={styles.createItineraryButtonContainer}>
          <button className={`${styles.createItineraryButton} ${disabled? styles.disabled:""}`} 
          disabled={disabled} onClick={generateResponse}>
            {props.createButtonText}</button>
        </div>
      );
}
