...
import { UserAccess, UserAccessWithDocId, ItineraryAccess } from '../../editFormTypeDefs';
import { SetterOrUpdater } from 'recoil';

export const fetchSharedItinerariesItinView = async (
  itineraryId: string,
 ) => {
  // Reference to Firestore collection
  try {
    const q = query(collection(db, 'ItineraryAccess'), where('itineraryId', '==', itineraryId));
